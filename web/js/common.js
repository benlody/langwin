...
$(window).load(function() {
	...
	//searchbar
	$('.menu-search .menu-search-btn').click(function (e) {
        
		$(this).toggleClass('close');
        $(this).siblings('.menu-search-form').toggleClass('show');
    });
	
	//form check radio trigger
	$('.control--checkbox').click(function (e) {
		
		if($(this).find('input').attr('type') == 'radio'){
			
			if($(this).find('input').prop("checked") == true && $(this).hasClass('input-radcheck-trigger')){
				$(this).parents().eq(0).find('.input-radcheck').prop('disabled', false);
			}
			else
				$(this).parents().eq(1).find('.input-radcheck').prop('disabled', true);
		}
		
		else{
			if($(this).hasClass('input-radcheck-trigger')){
				if($(this).find('input').prop("checked") == true)
					$(this).parents().eq(0).find('.input-radcheck').prop('disabled', false);
				else
					$(this).parents().eq(0).find('.input-radcheck').prop('disabled', true);
			}
		}
